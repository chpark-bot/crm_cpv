...
if uploaded_file is not None:
    ...
    st.header(":trophy: 이벤트 TOP 랭킹 분석")
    
    # 랭킹 테이블 출력을 위한 포맷된 컬럼 생성
    event_analysis['조회수 증감량 (랭킹용)'] = event_analysis.apply(
        lambda row: f"{int(row['조회수 증감액']):,} ({calculate_rate_str(row['현재 조회 수'], row['이전 조회 수'], row['조회수 증감액'])})", 
        axis=1
    )
    event_analysis['CPV 매출 증감액 (랭킹용)'] = event_analysis.apply(
        lambda row: f"{int(row['매출 증감액']):,} 원 ({calculate_rate_str(row['현재 매출'], row['이전 매출'], row['매출 증감액'])})", 
        axis=1
    )
    ...
    ranking_cols_views_change = ['이벤트명', '병원명', '조회수 증감량 (랭킹용)']
    ranking_cols_revenue_change = ['이벤트명', '병원명', 'CPV 매출 증감액 (랭킹용)']
    ...
    with col4:
        st.subheader("총 CPV 매출 TOP 3 이벤트")
    ...
    with col5:
        st.subheader("조회수 급상승 TOP 3 이벤트")
        # '조회수 증감액'을 기준으로 내림차순 정렬
        top3_views_change = event_analysis.sort_values(by='조회수 증감액', ascending=False).head(3)[ranking_cols_views_change]
        st.dataframe(top3_views_change, use_container_width=True, hide_index=True)
    
    with col6:
        st.subheader("CPV 매출 급상승 TOP 3 이벤트")
        # '매출 증감액'을 기준으로 내림차순 정렬
        top3_revenue_change = event_analysis.sort_values(by='매출 증감액', ascending=False).head(3)[ranking_cols_revenue_change]
        st.dataframe(top3_revenue_change, use_container_width=True, hide_index=True)
